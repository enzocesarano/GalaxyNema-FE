...
const MyFavorites = ({ films }) => {
  ...
  return (
    <Col className="col-12 col-xl-6 p-0 m-0 h-100 colFavorite overflow-card mb-5">
      <h2 className="text-center text-secondary mb-5">PREFERITI</h2>
      {isLoading ? (
        <p className="text-secondary text-center ">Non hai ancora aggiunto film ai preferiti!</p>
      ) : (
        films.map((film) => (
          <div
            key={film.id_film}
            className="m-2 mb-3 border border-0 border-bottom border-secondary"
          >
            <Link
              ...
              <Card className="cards border-0 rounded-4 p-3 mb-3 bg-transparent flex-row">
                ...
                <Card.Body className="text-secondary w-75 m-0 p-0 flex-column justify-content-between">
                  <div className="d-flex px-md-5 mb-4">
                    <div>
                      <p className="textTitle m-0 mb-1 me-3">{film.titolo}</p>
